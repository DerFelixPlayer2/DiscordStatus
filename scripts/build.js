...
const linux = process.env.XDG_CONFIG_HOME
...
for (let f = 0; f < list.length; f++) {
	...
	fs.copyFileSync(
		path.join(__dirname, '..', 'src', pluginName, 'config.json'),
		configPath
	);
	...
	if (libConfig.addInstallScript)
		result += require(path.join(__dirname, 'installscript.js'));
	result += template
...
function buildMeta(config) {
	...
	line(
		'authorId',
		config.info.authors[0].id ?? config.info.authors[0].discord_id
	);
